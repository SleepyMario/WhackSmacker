...
test("real PostgreSQL migrations, sessions, and two-user state are isolated", { skip: !process.env.TEST_DATABASE_URL }, async () => {
  ...
  try {
    ...
    assert.equal((await selectedPackages(pool, b.id)).length, 0);
    await selectPackage(pool, b.id, "com.example.curriculum", "1.0.0");
    ...
    const server = await startWebServer({ host: "127.0.0.1", port: 0, databaseUrl: process.env.TEST_DATABASE_URL, secureCookies:true });
    ...
    assert.equal((await userSettings(pool, a.id)).locale, "zh-Hant-TW");
    assert.equal((await userSettings(pool, b.id)).locale, "en-US");
    ...
    const restarted = await startWebServer({ host: "127.0.0.1", port: 0, databaseUrl: process.env.TEST_DATABASE_URL });
