...
const node={id:'chapter-003',label:'Chapter III — Checking Study Supplies',kind:'message',authoredReadingDirectory:directory,previewArtworkPath:directory+'media/scene.png'};
const marker='[[WHACKSMACKER_REVIEW_ARTWORK_REGION]]';
test('Chapter III scene stays directly after the title across reading controls',async()=>{
...
test('Chapter III scene uses preview lifecycle and clears on scroll or chapter change',async()=>{
 const calls=[];
 ...
 assert.equal((await manager.syncPreview(node,text)).rendered,true);
 assert.equal((await manager.syncPreview({id:'chapter-002',label:'Chapter IIII',kind:'message'},'Chapter IIII')).rendered,false);
