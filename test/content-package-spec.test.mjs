...
const koreanExampleUrl = new URL("../docs/content-packages/examples/korean-manifest.example.json", import.meta.url);
...
test("Korean example manifest is valid", async () => {
  assertValid(await readJson(koreanExampleUrl));
});
...
test("duplicate file paths fail", async () => {
  ...
  assertInvalid(manifest, /Duplicate file path: content\/content\.json/);
});
...
test("duplicate dependencies fail", async () => {
  const manifest = await readJson(koreanExampleUrl);
  manifest.dependencies.push({ ...manifest.dependencies[0] });

  assertInvalid(manifest, /Duplicate dependency: com\.sleepymario\.language\.linguistic-terminology/);
});
