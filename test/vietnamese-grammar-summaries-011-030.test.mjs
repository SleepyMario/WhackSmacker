...
for (let start = 11; start <= 26; start += 5) {
  const end = start + 4;
  test(`Vietnamese Grammar Easy/Hard ${start}-${end} have identical IDs and patterns`, async () => {
    ...
    const ids = (text) => [...text.matchAll(/^- (VIE-GRAMMAR-\d{3}[A-Z]?) -- (.+)$/gmu)].map((match) => [match[1], match[2]]);
    assert.deepEqual(ids(easy), ids(hard));
    assert.equal(ids(easy).length, start <= 21 ? 5 : 7);
  });
}
