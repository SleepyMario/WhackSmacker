...
const node={id:'chapter-004',label:'Chapter IV — Ren’s Study Materials',kind:'message',authoredReadingDirectory:directory,previewArtworkPath:directory+'media/scene.png'};
const marker='[[WHACKSMACKER_REVIEW_ARTWORK_REGION]]';
test('Chapter IV scene stays directly after the title across reading controls',async()=>{
...
test('Chapter IV scene uses preview lifecycle and clears on scroll or chapter change',async()=>{
 const calls=[];
 ...
 assert.equal((await manager.syncPreview(node,text)).rendered,true);
 assert.equal((await manager.syncPreview({id:'chapter-004',label:'Chapter IVI',kind:'message'},'Chapter IVI')).rendered,false);
