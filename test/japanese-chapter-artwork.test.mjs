...
const node={id:'chapter-001',label:'Chapter I — A First Meeting',kind:'message',authoredReadingDirectory:directory,previewArtworkPath:directory+'media/scene.png'};
const marker='[[WHACKSMACKER_REVIEW_ARTWORK_REGION]]';
test('Chapter I scene stays directly after the title across reading controls',async()=>{
...
test('Chapter I scene uses preview lifecycle and clears on scroll or chapter change',async()=>{
 const calls=[];
 ...
 assert.equal((await manager.syncPreview(node,text)).rendered,true);
 assert.equal((await manager.syncPreview({id:'chapter-002',label:'Chapter II',kind:'message'},'Chapter II')).rendered,false);
