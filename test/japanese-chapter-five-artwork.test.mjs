...
const node={id:'chapter-005',label:'Chapter V — At the Café',kind:'message',authoredReadingDirectory:directory,previewArtworkPath:directory+'media/scene.png'};
const marker='[[WHACKSMACKER_REVIEW_ARTWORK_REGION]]';
test('Chapter V scene stays directly after the title across reading controls',async()=>{
...
test('Chapter V scene uses preview lifecycle and clears on scroll or chapter change',async()=>{
 const calls=[];
 ...
 assert.equal((await manager.syncPreview(node,text)).rendered,true);
 assert.equal((await manager.syncPreview({id:'chapter-002',label:'Chapter VI',kind:'message'},'Chapter VI')).rendered,false);
