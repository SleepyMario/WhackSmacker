...
test("Chapters 1-3 retain the original Sino-Vietnamese source and section formatting", async () => {
  const expectedChapters = new Map([
    ["../vietnamese-curriculum/units/vietnamese-core/chapter-001-basic-sentences-1/chapter.md", "11d709592bf87560070d65afa260876f2ea1b849862bd4957dccc261f92f13fe"],
    ["../vietnamese-curriculum/units/vietnamese-core/chapter-002-basic-sentences-2/chapter.md", "8a71ebbdf4f33ec163aa59b1ef42413e8340d43585c9d1050f7f29ee5afe751b"],
    ["../vietnamese-curriculum/units/vietnamese-core/chapter-003-basic-sentences-3/chapter.md", "853675d28a16df5d81697dda5d52aa7c69cefee6de490fe5e7ff30bbeecec64a"]
  ]);
...
test("complete Sino-Vietnamese inventory resolves to canonical senses and earliest learner-facing evidence", async () => {
  ...
  assert.deepEqual(assertSinoVietnameseLexicon(lexicon, { canonicalBySenseId }), { lexicalSenseCount: 109, constituentMorphemeCount: 156, chapterCount: 45 });
...
test("Chapters 4-50 use sections only for eligible newly introduced senses", async () => {
  ...
  assert.deepEqual(audit.chapters_without_sections, [4, 14, 15, 27, 45]);
...
test("Sino-Vietnamese metadata leaves lexical topics, reviews, and the Chapter 50 boundary unchanged", async () => {
  ...
  assert.equal(topics.topics.length, 34);
  assert.equal(JSON.stringify(topics).includes("sino-vietnamese"), false);
  assert.deepEqual(lexicalAudit.review_findings.map((finding) => finding.card_count), [64, 72, 92, 80, 80, 82, 80, 88, 112, 84]);
