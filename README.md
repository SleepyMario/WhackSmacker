...
The private application currently exposes the preferred responsive shell, curriculum reader, and Review workflow. The reader supports exact authorized package/version selection, numerically ordered chapters, stable deep links and browser history, Normal/Expert/Developer projections, Translation/Characters/Breakdown controls, English and Traditional Chinese source overlays, and light/dark themes. Review discovers separately installed ordinary and specialized Review packages by authorized exact physical package ID/version while retaining stable card identities for progress; it uses the shared scheduler and progress stores, requires an explicit answer reveal, and accepts Again/Hard/Good/Easy by mouse, touch, or keys 1–4. Package management, progress dashboards, backup/restore, and subject workbenches remain visibly disabled.
