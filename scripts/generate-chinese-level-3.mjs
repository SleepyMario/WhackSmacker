...
const deckRoot = join(root, "review-content", "chinese-simplified-traditional", "level-3");
...
const [header, ...body] = (await readFile(sourcePath, "utf8"))
  ...
  .split("\n")
  .map((line) => line.split("\t"));
if (JSON.stringify(header) !== JSON.stringify(expectedHeader)) throw new Error(`Unexpected Level III mapping header in ${sourcePath}`);
if (body.length !== 466) throw new Error(`Expected 466 mappings, found ${body.length}`);
...
for (const row of body) {
  ...
  if (level !== "3") throw new Error(`Expected Level III mapping, found level ${level} at ${mappingCode}`);
  ...
  if (!simplifiedSentence.includes(simplified) || !traditionalSentence.includes(traditional)) {
    throw new Error(`Example sentences do not contain their mapped characters at ${mappingCode}`);
  }
  ...
  const commonTags = ["chinese", "simplified-traditional", "tghz2013", "level-3", "bidirectional", "one-context-sentence"];
  if (fieldUsage) commonTags.push("corner-case");
  const provenanceEvidence = `${simplifiedSentence} ⇄ ${traditionalSentence}`;
  const sourceDetails = [
    `context ${exampleSource}`, `source ${originalExampleSource}`,
    inherited === "true" ? "inherited mapping" : "official mapping", contextKind, fieldUsage
  ].filter(Boolean).join("; ");
  const directions = [
    {
  ...
  for (const direction of directions) {
    cards.push([
      `zh-tghz2013-level-3/${officialId}-map-${mapId}/${direction.suffix}`,
      "通用规范汉字表 第3级", "vocabulary", officialNo,
...
console.log(`Wrote ${cards.length - 1} cards for ${body.length} Level III mappings to ${outputPath}`);
