...
const mappingsPath = join(root, "review-content", "chinese-simplified-traditional", "level-3", "sources", "mappings.tsv");
const provenancePath = join(root, "review-content", "chinese-simplified-traditional", "level-3", "sources", "example-provenance.tsv");
const levelOneVocabularyPath = join(root, "review-content", "chinese-simplified-traditional", "level-1-vocabulary", "sources", "vocabulary.tsv");
const levelTwoVocabularyPath = join(root, "review-content", "chinese-simplified-traditional", "level-2-vocabulary", "sources", "vocabulary.tsv");
const outputPath = join(root, "review-content", "chinese-simplified-traditional", "level-3-vocabulary", "sources", "vocabulary.tsv");
const exclusionsPath = join(root, "review-content", "chinese-simplified-traditional", "level-3-vocabulary", "sources", "prior-level-exclusions.tsv");
...
const levelOneVocabulary = parseTsv(await readFile(levelOneVocabularyPath, "utf8"));
const levelTwoVocabulary = parseTsv(await readFile(levelTwoVocabularyPath, "utf8"));
const priorBySimplified = new Map([
  ...levelOneVocabulary.rows.map((row) => [row[levelOneVocabulary.at.simplified], {
    level: "Level I",
    order: row[levelOneVocabulary.at.order],
    traditional: row[levelOneVocabulary.at.traditional],
  }]),
  ...levelTwoVocabulary.rows.map((row) => [row[levelTwoVocabulary.at.simplified], {
    level: "Level II",
    order: row[levelTwoVocabulary.at.order],
    traditional: row[levelTwoVocabulary.at.traditional],
  }]),
]);
...
const excludedPrior = new Map();

// CC-CEDICT deliberately lists every reading and sense. These entries need the
// reading and short gloss that match the particular Level III source sentence.
const vocabularyOverrides = new Map(Object.entries({
  ...
  "寒伧": { pinyin: "han2 chen5", meaning: "shabby; unsightly" },
  "五笔": { pinyin: "wu3 bi3", meaning: "five brushstrokes" },
  "陇西": { pinyin: "Long3 xi1", meaning: "Longxi" },
  "井陉": { pinyin: "Jing3 xing2", meaning: "Jingxing" },
  "蚌埠": { pinyin: "Beng4 bu4", meaning: "Bengbu" },
  "建瓯": { pinyin: "Jian4 ou1", meaning: "Jian'ou" },
  "宝丰": { pinyin: "Bao3 feng1", meaning: "Baofeng" },
  "枣庄": { pinyin: "Zao3 zhuang1", meaning: "Zaozhuang" },
  "吉水": { pinyin: "Ji2 shui3", meaning: "Jishui" },
  "泸州": { pinyin: "Lu2 zhou1", meaning: "Luzhou" },
  "山东": { pinyin: "Shan1 dong1", meaning: "Shandong" },
  "通江": { pinyin: "tong1 jiang1", meaning: "to connect to a river" },
  "荥阳": { pinyin: "Xing2 yang2", meaning: "Xingyang" },
  "紫阳": { pinyin: "Zi3 yang2", meaning: "Ziyang (given name)" },
  "鲁山": { pinyin: "Lu3 shan1", meaning: "Lushan" },
  "吉林": { pinyin: "Ji2 lin2", meaning: "Jilin" },
  "杭州": { pinyin: "Hang2 zhou1", meaning: "Hangzhou" },
  "临安": { pinyin: "Lin2 an1", meaning: "Lin'an" },
  "江口": { pinyin: "Jiang1 kou3", meaning: "Jiangkou" },
  "甘肃": { pinyin: "Gan1 su4", meaning: "Gansu" },
  "四川": { pinyin: "Si4 chuan1", meaning: "Sichuan" },
  "邯郸": { pinyin: "Han2 dan1", meaning: "Handan" },
  "曲阜": { pinyin: "Qu1 fu4", meaning: "Qufu" },
  "榴梿": { pinyin: "liu2 lian2", meaning: "durian" },
  "莱阳": { pinyin: "Lai2 yang2", meaning: "Laiyang" },
  "常州": { pinyin: "Chang2 zhou1", meaning: "Changzhou" },
  "渑池": { pinyin: "Mian3 chi2", meaning: "Mianchi" },
  "桃源": { pinyin: "Tao2 yuan2", meaning: "Taoyuan" },
  "曲阳": { pinyin: "Qu1 yang2", meaning: "Quyang" },
  "平江": { pinyin: "Ping2 jiang1", meaning: "Pingjiang" },
  "阖家": { pinyin: "he2 jia1", meaning: "the whole family" },
  "承德": { pinyin: "Cheng2 de2", meaning: "Chengde" },
  "令嫒": { pinyin: "ling4 ai4", meaning: "your daughter (polite)" },
  "浙江": { pinyin: "Zhe4 jiang1", meaning: "Zhejiang" },
  "丽水": { pinyin: "Li4 shui3", meaning: "Lishui" },
  "长沙": { pinyin: "Chang2 sha1", meaning: "Changsha" },
  "琅琊": { pinyin: "Lang2 ya2", meaning: "Langya" },
  "真珠": { pinyin: "zhen1 zhu1", meaning: "pearl" },
  "蝼蚁": { pinyin: "lou2 yi3", meaning: "an insignificant person" },
  "南阳": { pinyin: "Nan2 yang2", meaning: "Nanyang" },
  "湖北": { pinyin: "Hu2 bei3", meaning: "Hubei" },
  "鹬蚌相争": { pinyin: "yu4 bang4 xiang1 zheng1", meaning: "a snipe and clam quarrel" },
  "岳阳": { pinyin: "Yue4 yang2", meaning: "Yueyang" },
  "酦酵": { pinyin: "fa1 jiao4", meaning: "fermentation" },
}));

// Where the MOE dictionary records several readings, these select the one
// matching the meaning used by the Level III sentence.
...
function traditionalFormInExample(simplifiedSentence, traditionalSentence, word, dictionaryForm) {
  ...
  if (simplifiedCharacters.length !== traditionalCharacters.length) return undefined;
...
for (const row of mappings.rows) {
  ...
  for (const word of words) {
    ...
    if (traditional === undefined || !traditionalSentence.includes(traditional)) continue;
    const priorEntry = priorBySimplified.get(word);
    if (priorEntry !== undefined) {
      if (!excludedPrior.has(word)) {
        excludedPrior.set(word, {
          simplified: word,
          traditional,
          priorLevel: priorEntry.level,
          priorTraditional: priorEntry.traditional,
          priorOrder: priorEntry.order,
          firstLevelThreeMapping: mappingCode,
        });
      }
      continue;
    }
...
const exclusionHeader = ["simplified", "traditional_in_level_3", "prior_level", "traditional_in_prior_level", "prior_level_order", "first_level_3_mapping"];
const exclusionRows = [...excludedPrior.values()].map((entry) => [
  entry.simplified, entry.traditional, entry.priorLevel, entry.priorTraditional, entry.priorOrder, entry.firstLevelThreeMapping
]);
await writeFile(exclusionsPath, [exclusionHeader, ...exclusionRows].map((row) => row.join("\t")).join("\n") + "\n", "utf8");
console.log(`Wrote ${outputRows.length} distinct Level III-only vocabulary entries to ${outputPath}`);
console.log(`Excluded ${exclusionRows.length} Level I/II vocabulary entries and recorded them in ${exclusionsPath}`);
