...
for (const row of mappings.rows) {
  ...
  if (!example) continue;
  row[mappings.at.simplified_example] = example[examples.at.simplified_word];
  row[mappings.at.traditional_example] = example[examples.at.traditional_word];
  row[mappings.at.example_source] = "authored-example";
  row[mappings.at.original_example_source] = "project-authored";
...
if (replaced !== 146) throw new Error(`Expected 146 replacements, made ${replaced}`);
...
console.log(`Replaced ${replaced} Level III placeholder pairs; no placeholder examples remain.`);
