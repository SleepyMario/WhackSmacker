...
const deckRoot = join(root, "review-content", "chinese-simplified-traditional", "level-3-vocabulary");
...
if (body.length !== 39) throw new Error(`Expected 39 Level III-only logical entries, found ${body.length}`);
...
for (const row of body) {
  ...
  const lexicalId = `zh.level-3-vocabulary.${id}`;
  const sourceDescription = exampleSource === "tatoeba"
    ? `Tatoeba Mandarin sentence ${exampleSourceId}`
    : exampleSource === "authored" ? "project-authored Level III example" : "reviewed Level III example";
  const common = {
    deck: "Level III - Vocabulary",
    kind: "vocabulary",
    sourceUnit: `Level III mapping ${mappingCode}`,
    distractors: [],
    explanation: "",
    ...
    tags: ["chinese", "vocabulary", "level-3", "abc", "simplified", "traditional"]
  };
  const directions = [
  ...
  for (const direction of directions) {
    cards.push([
      `zh-level-3-vocabulary/${id}/${direction.suffix}`, common.deck, common.kind, common.sourceUnit,
