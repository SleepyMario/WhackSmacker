...
const deckRoot = join(root, "review-content", "chinese-simplified-traditional", "level-1-vocabulary");
...
if (body.length !== 2815) throw new Error(`Expected 2815 logical entries, found ${body.length}`);
...
for (const row of body) {
  ...
  const lexicalId = `zh.level-1-vocabulary.${id}`;
  const sourceDescription = exampleSource === "tatoeba"
    ? `Tatoeba Mandarin sentence ${exampleSourceId}`
    : exampleSource === "authored" ? "project-authored Level I example" : "reviewed Level I example";
  const common = {
    deck: "Level I - Vocabulary",
    kind: "vocabulary",
    sourceUnit: `Level I mapping ${mappingCode}`,
    distractors: [],
    explanation: "",
    ...
    tags: ["chinese", "vocabulary", "level-1", "abc", "simplified", "traditional"]
  };
  const directions = [
  ...
  for (const direction of directions) {
    cards.push([
      `zh-level-1-vocabulary/${id}/${direction.suffix}`, common.deck, common.kind, common.sourceUnit,
