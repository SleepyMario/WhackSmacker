...
const mappingsPath = join(root, "review-content", "chinese-simplified-traditional", "level-2", "sources", "mappings.tsv");
const provenancePath = join(root, "review-content", "chinese-simplified-traditional", "level-2", "sources", "example-provenance.tsv");
...
const outputPath = join(root, "review-content", "chinese-simplified-traditional", "level-2-vocabulary", "sources", "vocabulary.tsv");
const exclusionsPath = join(root, "review-content", "chinese-simplified-traditional", "level-2-vocabulary", "sources", "level-1-exclusions.tsv");
...
const levelOneBySimplified = new Map(levelOneVocabulary.rows.map((row) => [row[levelOneVocabulary.at.simplified], {
...
const excludedLevelOne = new Map();

// CC-CEDICT deliberately lists every reading and sense. These entries need the
// reading and short gloss that match the particular Level II source sentence.
...
for (const row of mappings.rows) {
  ...
  for (const word of words) {
    ...
    const levelOneEntry = levelOneBySimplified.get(word);
    if (levelOneEntry !== undefined) {
      if (!excludedLevelOne.has(word)) {
        excludedLevelOne.set(word, {
          simplified: word,
          traditional,
          levelOneTraditional: levelOneEntry.traditional,
          levelOneOrder: levelOneEntry.order,
          firstLevelTwoMapping: mappingCode,
        });
      }
      continue;
    }
...
const exclusionHeader = ["simplified", "traditional_in_level_2", "traditional_in_level_1", "level_1_order", "first_level_2_mapping"];
const exclusionRows = [...excludedLevelOne.values()].map((entry) => [
  entry.simplified, entry.traditional, entry.levelOneTraditional, entry.levelOneOrder, entry.firstLevelTwoMapping
]);
await writeFile(exclusionsPath, [exclusionHeader, ...exclusionRows].map((row) => row.join("\t")).join("\n") + "\n", "utf8");
console.log(`Wrote ${outputRows.length} distinct Level II-only vocabulary entries to ${outputPath}`);
console.log(`Excluded ${exclusionRows.length} Level I vocabulary entries and recorded them in ${exclusionsPath}`);
