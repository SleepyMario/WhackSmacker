...
const deckRoot = join(root, "review-content", "chinese-simplified-traditional", "level-2");
...
if (JSON.stringify(header) !== JSON.stringify(expectedHeader)) {
  throw new Error(`Unexpected Level II mapping header in ${sourcePath}`);
}
if (body.length !== 927) throw new Error(`Expected 927 mappings, found ${body.length}`);
...
for (const row of body) {
  ...
  if (level !== "2") throw new Error(`Expected Level II mapping, found level ${level} at ${mappingCode}`);
  ...
  const examplesContainContexts = simplifiedSentence.includes(simplifiedWord) && traditionalSentence.includes(traditionalWord);
  const mappedCharacterExample = simplifiedSentence.includes(simplified)
    && traditionalSentence.includes(traditional);
  if (!examplesContainContexts && !mappedCharacterExample) {
    throw new Error(`Example sentences do not contain their context words or mapped characters at ${mappingCode}`);
  }
  ...
  const commonTags = [
    "chinese", "simplified-traditional", "tghz2013", "level-2", "bidirectional", "one-context-sentence"
  ];
  ...
  for (const direction of directions) {
    cards.push([
      `zh-tghz2013-level-2/${officialId}-map-${mapId}/${direction.suffix}`,
      "通用规范汉字表 第2级", "vocabulary", officialNo,
...
console.log(`Wrote ${cards.length - 1} cards for ${body.length} Level II mappings to ${outputPath}`);
