...
const sourcesRoot = join(root, "review-content", "chinese-simplified-traditional", "level-2", "sources");
...
for (const row of mappings.rows) {
  ...
  if (!example) continue;
  const simplifiedWord = example[examples.at.simplified_word];
  const traditionalWord = example[examples.at.traditional_word];
  if (!simplifiedWord.includes(row[mappings.at.simplified]) || !traditionalWord.includes(row[mappings.at.traditional])) {
    throw new Error(`Replacement context does not contain its mapped character at ${code}`);
  }
  // Level II's inherited source contains a small number of poor automatic
  // context selections.  The reviewed provenance file may therefore replace
  // the context word as well as the sentence.
  row[mappings.at.simplified_example] = simplifiedWord;
  row[mappings.at.traditional_example] = traditionalWord;
  const sourceType = example[examples.at.source_type];
  row[mappings.at.example_source] = ["tatoeba", "leipzig"].includes(sourceType) ? "corpus-example" : "authored-example";
  row[mappings.at.original_example_source] = sourceType === "authored"
    ? "project-authored"
    : `${sourceType}:${example[examples.at.source_id]}`;
