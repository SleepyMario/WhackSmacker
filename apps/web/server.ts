...
import { assertDatabaseReady, authenticateUser, createDatabasePool, createSession, csrfMatches, databaseConfig, ensureUserReviewState, loadUserReviewStates, recordUserReview, redactDatabaseError, resolveSession, revokeSession, selectedPackages, selectPackage, unselectPackage, updateUserSettings, userSettings, type DatabaseSession } from "../../packages/storage/postgres";
...
async function handle(request: IncomingMessage, response: ServerResponse, options: WebServerOptions, context: WebContext): Promise<void> {
  try {
    ...
    if (url.pathname === "/api/login" && request.method === "POST") return login(request, response, options, context);
    ...
    const identity=await authorized(request,options,context);if(identity===false){response.setHeader("WWW-Authenticate",'Session realm="WhackSmacker"');return json(response,401,{error:"Authentication required."})}
    ...
    if (url.pathname === "/api/logout" && request.method === "POST") return logout(request,response,options,context,identity);
    ...
    if (url.pathname === "/api/settings" && request.method === "PUT") return updateSettings(request, response, options,context.pool,identity);
    if (url.pathname === "/api/packages/install" && request.method === "POST") return await install(request, response, options,context,identity);
    if (url.pathname === "/api/packages/remove" && request.method === "POST") return remove(request, response, options,context.pool,identity);
    if (url.pathname === "/api/review/answer" && request.method === "POST") return answer(request, response, options,context.pool,identity);
    if (url.pathname === "/api/review-items" && request.method === "GET") return reviewItems(url, response, options,context.pool,identity);
    if (url.pathname === "/api/content" && request.method === "GET") return content(url, response, options,context.pool,identity);
    if (url.pathname === "/api/content/entry" && request.method === "GET") return contentEntry(url, response, options,context.pool,identity);
  ...
  } catch (error) { const databaseFailure=context.pool&&isDatabaseError(error);json(response,databaseFailure?500:400,{error:databaseFailure?"Database request failed.":redactDatabaseError(error)}); }
}
...
async function databaseState(options:WebServerOptions,pool:Pool,user:DatabaseSession){const settings=await userSettings(pool,user.id),selected=await selectedPackages(pool,user.id),keys=new Set(selected.map(x=>x.package_id+"@"+x.package_version)),installed=(await listInstalledContentPackages(options.dataDir)).filter(x=>keys.has(x.packageId+"@"+x.packageVersion)&&x.contentType!=="curriculum-source-language-pack"),available=options.cataloguePath?(await listAvailableContentPackages(options.cataloguePath)).filter(x=>x.contentType!=="curriculum-source-language-pack"):[],now=new Date().toISOString().replace(/\.\d{3}Z$/u,"Z"),items=(await listReadingReviewItems({dataDir:options.dataDir,sourceLocale:settings.locale})).filter(x=>keys.has(x.packageId+"@"+x.packageVersion));let states=await loadUserReviewStates(pool,user.id),stateKeys=new Set(states.map(reviewIdentityKey));for(const x of items){const identity={packageId:x.packageId,packageVersion:x.packageVersion,...(x.sourcePath?{sourcePath:x.sourcePath}:{}),itemId:x.item.id};if(!stateKeys.has(reviewIdentityKey(identity))){states.push(await ensureUserReviewState(pool,user.id,identity,now));stateKeys.add(reviewIdentityKey(identity))}}const byKey=new Map(states.map(x=>[reviewIdentityKey(x),x])),sources=(await listReadingReviewSources({dataDir:options.dataDir,sourceLocale:settings.locale})).filter(x=>keys.has(x.packageId+"@"+x.packageVersion)),decks=sources.map(source=>{const deckItems=items.filter(x=>x.packageId===source.packageId&&x.sourcePath===source.sourcePath),ss=deckItems.map(x=>byKey.get(reviewIdentityKey({packageId:x.packageId,packageVersion:x.packageVersion,...(x.sourcePath?{sourcePath:x.sourcePath}:{}),itemId:x.item.id}))),reviewed=ss.filter(x=>(x?.reviewCount??0)>0).length,due=ss.filter(x=>x&&x.nextReviewAt<=now).length;return{...source,title:source.title??source.sourcePath,itemCount:deckItems.length,reviewed,due,status:reviewed===0?"not_started":reviewed===deckItems.length&&due===0?"finished":due>0?"has_cards_to_review":"no_cards_to_review"}});return{locale:settings.locale,theme:settings.theme,user:{username:user.username,role:user.role},csrfToken:undefined,installed,available,decks,review:{total:states.length,due:states.filter(x=>x.nextReviewAt<=now).length,reviewed:states.filter(x=>x.reviewCount>0).length}}}

async function updateSettings(req:IncomingMessage,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const body=await bodyJson(req);if(!isSourceLocale(body.locale))throw new Error("Unsupported source language.");const theme=body.theme===undefined?undefined:String(body.theme);if(theme!==undefined&&theme!=="light"&&theme!=="dark")throw new Error("Unsupported theme.");if(pool)await updateUserSettings(pool,(id as DatabaseSession).id,body.locale,theme);else await saveSourceLanguage(body.locale,settingsDir(options));json(res,200,{locale:body.locale,...(theme?{theme}:{})})}
async function install(req:IncomingMessage,res:ServerResponse,options:WebServerOptions,context:WebContext,id:true|DatabaseSession){if(!options.cataloguePath)throw new Error("Start with --catalogue.");const body=await bodyJson(req);if("user_id"in body)throw new Error("user_id is not accepted.");const result=await exclusivePackageMutation(context,()=>installContentPackage({cataloguePath:options.cataloguePath!,packageId:required(body.packageId),dataDir:options.dataDir}));if(context.pool)await selectPackage(context.pool,(id as DatabaseSession).id,result.record.packageId,result.record.packageVersion);json(res,200,result)}
...
async function answer(req:IncomingMessage,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const b=await bodyJson(req),rating=String(b.rating) as ReviewRating;if(!["again","hard","good","easy"].includes(rating))throw new Error("Invalid rating.");const identity={packageId:required(b.packageId),packageVersion:required(b.packageVersion),sourcePath:required(b.sourcePath),itemId:required(b.itemId)},at=new Date().toISOString().replace(/\.\d{3}Z$/u,"Z");if(pool){const user=id as DatabaseSession;if(!await allowedPackage(pool,user.id,identity.packageId))return json(res,404,{error:"Package not selected."});const locale=(await userSettings(pool,user.id)).locale,items=await listReadingReviewItems({dataDir:options.dataDir,packageId:identity.packageId,sourceLocale:locale});if(!items.some(x=>x.packageVersion===identity.packageVersion&&x.sourcePath===identity.sourcePath&&x.item.id===identity.itemId))return json(res,404,{error:"Review item not found."});return json(res,200,await recordUserReview(pool,user.id,identity,rating,at))}json(res,200,await recordReadingReviewAnswer({dataDir:options.dataDir,...identity,rating,reviewedAt:at}))}
async function allowedPackage(pool:Pool,userId:string,packageId:string){return(await selectedPackages(pool,userId)).some(x=>x.package_id===packageId)}
async function reviewItems(url:URL,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const packageId=required(url.searchParams.get("packageId"));if(pool&&!await allowedPackage(pool,(id as DatabaseSession).id,packageId))return json(res,404,{error:"Package not selected."});const locale=pool?(await userSettings(pool,(id as DatabaseSession).id)).locale:(await loadSourceLanguageSettings(settingsDir(options))).sourceLanguage;json(res,200,{items:await listReadingReviewItems({dataDir:options.dataDir,packageId,sourceLocale:locale})})}
async function content(url:URL,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const locale=pool?(await userSettings(pool,(id as DatabaseSession).id)).locale:(await loadSourceLanguageSettings(settingsDir(options))).sourceLanguage,all=await listInstalledReadablePackages(options.dataDir,locale),allowed=pool?new Set((await selectedPackages(pool,(id as DatabaseSession).id)).map(x=>x.package_id)):undefined,packages=allowed?all.filter(x=>allowed.has(x.packageId)):all,selected=url.searchParams.get("packageId");if(selected&&allowed&&!allowed.has(selected))return json(res,404,{error:"Package not selected."});json(res,200,{packages,entries:selected?await listReadableContentEntries(selected,options.dataDir,url.searchParams.get("version")??undefined):[]})}
async function contentEntry(url:URL,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const packageId=required(url.searchParams.get("packageId"));if(pool&&!await allowedPackage(pool,(id as DatabaseSession).id,packageId))return json(res,404,{error:"Package not selected."});const locale=pool?(await userSettings(pool,(id as DatabaseSession).id)).locale:(await loadSourceLanguageSettings(settingsDir(options))).sourceLanguage;json(res,200,await readInstalledContentEntry({dataDir:options.dataDir,packageId,packageVersion:url.searchParams.get("version")??undefined,path:required(url.searchParams.get("path")),locale}))}
