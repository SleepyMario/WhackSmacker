...
async function handle(request: IncomingMessage, response: ServerResponse, options: WebServerOptions, context: WebContext): Promise<void> {
  ...
  try {
    ...
    if (url.pathname === "/api/package-media" && request.method === "GET") return await packageMedia(url, request, response, options,context.pool,identity);
    ...
    const files: Record<string, [string, string]> = { "/app": ["index.html", "text/html; charset=utf-8"], "/app.js": ["app.js", "text/javascript; charset=utf-8"], "/memorization-media.js": ["memorization-media.js", "text/javascript; charset=utf-8"], "/styles.css": ["styles.css", "text/css; charset=utf-8"] };
...
async function reviewItems(url:URL,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const packageId=query(url,"packageId"),version=query(url,"version");if(pool&&!await allowedPackage(pool,(id as DatabaseSession).id,packageId,version))throw new HttpError(403,"This package version is not selected for your account.");const locale=pool?(await userSettings(pool,(id as DatabaseSession).id)).locale:(await loadSourceLanguageSettings(settingsDir(options))).sourceLanguage;const items=(await listReadingReviewItems({dataDir:options.dataDir,packageId,sourceLocale:locale})).filter(x=>x.packageVersion===version).map(item=>({...item,media:webReviewMedia(item)}));json(res,200,{items})}
function webReviewMedia(item:ReadingReviewItem){const contentPackageId=item.contentPackageId??item.packageId,seen=new Set<string>();return memorizationMediaReferences(item.item).flatMap(reference=>{if(seen.has(reference.path))return[];seen.add(reference.path);const query=new URLSearchParams({packageId:contentPackageId,version:item.packageVersion,path:reference.path});return[{path:reference.path,mediaType:reference.path.endsWith(".webp")?"image/webp":reference.path.endsWith(".png")?"image/png":"image/jpeg",url:`/api/package-media?${query}`} ]})}
async function packageMedia(url:URL,req:IncomingMessage,res:ServerResponse,options:WebServerOptions,pool:Pool|undefined,id:true|DatabaseSession){const packageId=query(url,"packageId"),version=query(url,"version"),path=query(url,"path");if(pool&&!await webMediaAllowed(pool,(id as DatabaseSession).id,packageId,version,options.dataDir))throw new HttpError(403,"This package version is not selected for your account.");let asset;try{asset=await readInstalledPackageImageAsset(packageId,version,path,options.dataDir)}catch(error){const message=error instanceof Error?error.message:"";throw new HttpError(/not found|missing|not a declared/iu.test(message)?404:400,"Package image is unavailable.")}res.writeHead(200,securityHeaders({"Content-Type":asset.mediaType,"Content-Length":String(asset.size),"Cache-Control":"private, no-store"}));if(req.method!=="HEAD")res.end(asset.data);else res.end()}
async function webMediaAllowed(pool:Pool,userId:string,packageId:string,version:string,dataDir?:string){if(await allowedPackage(pool,userId,packageId,version))return true;const record=(await listInstalledContentPackages(dataDir)).find(candidate=>candidate.packageId===packageId&&candidate.packageVersion===version);if(record===undefined)return false;for(const relatedId of record.relatedPackageIds??[])if(await allowedPackage(pool,userId,relatedId,version))return true;return false}
