  ...
  if (node.kind === "content") {
    ...
    if (node.readingSupportPath !== undefined) {
      try {
        ...
        readingSupport = parseSharedReadingSupport(supportResult.text);
      } catch {
    ...
    if (options.translationsEnabled === true && node.translationPath !== undefined) {
      try {
        ...
        translation = parseSharedStructuredReadingTranslation(translationResult.text);
      } catch {
        // A missing or damaged optional prototype translation must not hide the original reading.
      }
    }
    if (/\/chapter\.md$/u.test(node.filePath)) {
      return projectReadingChapterMarkdown(result.text, {
        mode: options.displayMode ?? defaultCurriculumDisplayMode,
    ...
    return projectCurriculumMarkdown(result.text, options.displayMode ?? defaultCurriculumDisplayMode, {
